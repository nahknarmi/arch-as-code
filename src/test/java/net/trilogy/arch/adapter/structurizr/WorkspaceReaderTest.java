...
public class WorkspaceReaderTest {
    ...
    public void shouldLoadViews() throws Exception {
        ...
        assertThat(systemViews.size(), equalTo(1));
        assertThat(systemViews.stream().map(View::getSoftwareSystemId).collect(toList()), hasItems("2"));

        Collection<ContainerView> containerViews = views.getContainerViews();
        assertThat(containerViews.size(), equalTo(1));
        assertThat(containerViews.stream().map(View::getSoftwareSystemId).collect(toList()), hasItem("2"));

        Collection<ComponentView> componentViews = views.getComponentViews();
        assertThat(componentViews.size(), equalTo(1));
        assertThat(componentViews.stream().map(ComponentView::getContainerId).collect(toList()), hasItem("20"));

        Collection<SystemLandscapeView> landscapeViews = views.getSystemLandscapeViews();
        assertThat(landscapeViews.size(), equalTo(1));

        Collection<DynamicView> dynamicViews = views.getDynamicViews();
        assertThat(dynamicViews.size(), equalTo(1));

        Collection<DeploymentView> deploymentViews = views.getDeploymentViews();
        assertThat(deploymentViews.size(), equalTo(2));
        assertThat(systemViews.stream().map(View::getSoftwareSystemId).collect(toList()), everyItem(equalTo("2")));
    }
}
